    ...
    var DrawState =
        ...
        updatecanvas();
            ...
            if (syncServer && drawObject.Tool != DrawTool.Pencil) {
                ...
                whiteboardHub.server.sendDraw(message,$("#sessinId").val(), $("#groupName").val());
            }
        }
//
function toggleBG1() {
...
function toggleBG2() {
    ...
    canvaso.width = canvas.width;
        ...
        window.onresize = function (event) {
            canvas.width = event.target.innerWidth - 50;
            canvas.height = event.target.innerHeight - (52 * 2.5);
            canvaso.width = event.target.innerWidth - 50;
            canvaso.height = event.target.innerHeight - (52 * 2.5);
        };
    ...
    curtop += e.offsetTop;
        ...
        var drawobjects = [];
            ...
            //    whiteboardHub.server.joinGroup(/*$("#groupName").val()*/"foo");
            //});

$.connection.hub.start().done(function () {
    whiteboardHub.server.joinGroup($("#groupName").val()).done(

       // function () { whiteboardHub.server.sendDraw($("#drawObject").val(), $("#groupName").val(), $("#sessinId").val()); }
    );
});
